...
services:
  ...
  traefik:
    ...
    volumes:
      ...
      - ./config/traefik/plugins/req-preprocess:/plugins-local/src/github.com/SwanHubX/req-preprocess
    healthcheck:
  ...
  clickhouse:
    <<: *common
    image: ccr.ccs.tencentyun.com/self-hosted/clickhouse:24.3-swanlab
  ...
  swanlab-server:
    <<: *common
    image: ccr.ccs.tencentyun.com/self-hosted/swanlab-server:v1-5
    container_name: swanlab-server
    volumes:
      - ./config/swanlab:/app/config
    depends_on:
      postgres:
  ...
  swanlab-house:
    <<: *common
    image: ccr.ccs.tencentyun.com/self-hosted/swanlab-house:v1-0
  ...
  swanlab-cloud:
    <<: *common
    image: ccr.ccs.tencentyun.com/self-hosted/swanlab-cloud:0.0.1
  ...
  swanlab-next:
    <<: *common
    image: ccr.ccs.tencentyun.com/self-hosted/swanlab-next:0.0.5
...
docker compose -p swanlab up -d

# init minio
sleep 10s
docker compose -p swanlab exec -it minio bash -c "
